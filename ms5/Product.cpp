...
namespace ama {
	...
	double Product::total_cost() const {
		double total = 0.0;
		if (taxable) {
			total = (p_bTax * (1.0 + tax_rate)) * qtyAvailable();
		}
		else {
			total = p_bTax * qtyAvailable();
		}
		return total;
	}
	...
	std::ostream & Product::write(std::ostream & out, int writeMode) const {
		if (!isClear()) {
			...
			if (writeMode == write_table) {
				out << " " << right << setw(max_length_sku) << p_sku;
				out << " | ";
				if (strlen(p_name) > 16) {
					char newName[14];
					newName[0] = '\0';
					strncpy(newName, p_name, 14);
					newName[13] = '\0';
					out << left << setw(13) << newName << "... | ";
				}
				else {
					out << left << setw(16) << p_name << " | ";
				}

				out << setw(10) << left << p_unit << " | " << setw(7) << right;
				out.setf(ios::fixed, ios::floatfield);
				out.precision(2);
				out << p_bTax << " | "
					<< right << setw(3) << (taxable == true ? "yes" : "no") << " | "
					<< right << setw(6) << qtyAvailable() << " | "
					<< right << setw(6) << p_needed << " |";
			...
			if (writeMode == write_human) {
				...
				out << setw(max_length_label) << right << "Price after Tax: " << fixed << (taxable == true ? p_bTax * (1.0 + tax_rate) : p_bTax) << endl;
